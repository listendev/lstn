...
func generatePackageLock(ctx context.Context, dir string) ([]byte, error) {
	...
	tmp, err := util.TempDir(fs.Current, "", "lstn-*")
	if err != nil {
	...
	defer util.RemoveAll(fs.Current, tmp)

	// Copy the package.json in the temporary directory
	packageJSONPath := filepath.Join(dir, "package.json")
	packageJSON, err := util.ReadFile(fs.Current, packageJSONPath)
	if err != nil {
	...
	packageLockJSON, _ := util.ReadFile(fs.Current, filepath.Join(tmp, "package-lock.json"))
