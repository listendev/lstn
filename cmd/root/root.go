...
func New(ctx context.Context) (*Command, error) {
	...
	var rootCmd = &cobra.Command{
		...
		PersistentPreRunE: func(c *cobra.Command, args []string) error {
			...
			if subgroup, hasSubgroup := c.Annotations["subgroup"]; hasSubgroup && subgroup == groups.WithDirectory.String() && len(args) > 0 {
	...
	// Fallback to the default subcommand (scan) when the user doesn't specify one explicitly.
